...
import React, { useState } from 'react';
import { useNavigate } from 'react-router-dom'; // <-- ADDED: For handling navigation
import { useAuth } from '@/contexts/AuthContext';
import AuthLayout from '@/components/AuthLayout';
import { Mail, Lock, ArrowRight, Github, Chrome, Eye, EyeOff, Loader2 } from 'lucide-react';

// This helper component does not need any changes.
const InputField = ({ icon: Icon, label, type, value, onChange, placeholder, id, autoComplete, required = true, children }) => (
  <div className="group">
    <label htmlFor={id} className="block text-sm font-semibold text-slate-700 mb-2">
      {label}
...
// This helper component does not need any changes.
const SocialButton = ({ icon: Icon, text, className = "" }) => (
    <button type="button" className={`group flex-1 inline-flex w-full items-center justify-center py-3 px-4 text-sm font-medium text-slate-700 bg-white/80 backdrop-blur-sm border border-slate-200 rounded-xl hover:bg-white hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-all duration-300 hover:-translate-y-0.5 ${className}`}>
      <Icon className="w-5 h-5 mr-2 text-slate-500 group-hover:scale-110 transition-transform" />
      <span className="group-hover:text-slate-800 transition-colors">{text}</span>
    </button>
);
...
const LoginPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // 2. Wait for the login function (with its 2-second delay) to finish
      await login(email, password);

      // 3. If the login was successful, THEN navigate.
      // The component will unmount, taking the loader with it.
      navigate('/dashboard');

    } catch (error) {
      console.error("Login failed:", error);
      // In a real app, show an error message to the user here.
      // 4. If login fails, turn the loader off so the user can try again.
      setIsLoading(false);
    }
  };
